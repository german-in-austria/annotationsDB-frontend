<template>
  <div class="selgroup">
    ...
    <!-- <ul v-if="menue.aInformant > 0" class="lmfa-l">
      <li v-for="aTranskript in menue.aTranskripte">
        <a href="#" v-on:click.prevent="getTranskript(aTranskript.model.pk)" v-bind:class="{ lmfabc: true, open: annotationsTool.aPK === aTranskript.model.pk }" :title="'PK: ' + aTranskript.model.pk + '\nUpdate: ' + aTranskript.model.update_time + ' Uhr'">${aTranskript.model.name}<span>${aTranskript.count.toLocaleString()}</span></a>
      </li>
    </ul> -->
  </div>
</template>

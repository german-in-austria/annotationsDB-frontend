<template>
  ...
  >
    <g class="zEventSetLine"
      ...
      :key="'zesl' + aEsdI"
    >
      <g :class="'zEventSet' + (transcript.hoveredEventSet === aEventSet.pk ? ' hover' : '') + ((aEventSet === transcript.selectedEventSet) ? ' selected' : '') /* ToDo: + ((transcript.selectedToken && transcript.selectedToken.tokenSetsList && transcript.selectedToken.tokenSetsList.indexOf(aEventSet) > -1) ? ' active' : '')*/"
        v-for="(aEventSet, aTsI) in aEventSetDeep"
        :key="aTsI"
        :transform="'translate(0,' + (aEsdI * aEventSetHeight) + ')'"
        @click="showaEventSetInfos(aEventSet, false, $event)"
        @mouseover="transcript.hoveredEventSet = aEventSet.pk"
        @mouseleave="transcript.hoveredEventSet = null"
      >
        <!-- Eventsets anzeigen -->
        <g class="zEsVB">
          <g :class="'zEsVBln dg' + dg" v-for="dg in [0,1]" :key="'zEsVBlndg' + dg">
            <path :d="'M' + (((typeof eventSetsSvgData[aEventSet.pk].startX === 'number') ? eventSetsSvgData[aEventSet.pk].startX + 1 : 0)) + ' ' +
                            ((typeof eventSetsSvgData[aEventSet.pk].startX === 'number') ? (-(eventSetsDeepList.length - 1 - aEsdI) * aEventSetHeight) : (aEventSetHeight / 2)) +
                      ((typeof eventSetsSvgData[aEventSet.pk].startX === 'number')
                        ? ' V' + (aEventSetHeight / 2 - 6) + 'a6,6 0 0 0 6,6 '
                        : '') +
                      ' H' + ((eventSetsSvgData[aEventSet.pk].endX) ? eventSetsSvgData[aEventSet.pk].endX - 7 : svgWidth) +
                      ((eventSetsSvgData[aEventSet.pk].endX) ? 'a6,6 0 0 0 6,-6 V' + (-(eventSetsDeepList.length - 1 - aEsdI) * aEventSetHeight) : '')" />
          </g>
          <text :x="(((eventSetsSvgData[aEventSet.pk].endX) ? eventSetsSvgData[aEventSet.pk].endX : svgWidth) + ((eventSetsSvgData[aEventSet.pk].startX) ? eventSetsSvgData[aEventSet.pk].startX : 0)) / 2"
                :y="0"
          ...
          >&nbsp;
            {{
              ...
              ? (transcript.showEventSetIds ? aEventSet.pk + ': ' : '') + globals.tagsData.data.tagsText(getFirstObjectOfValueInPropertyOfArray(aAntworten[aEventSet.aId].tags, 'e', transcript.previewTagEbene).tags)
              : (
                transcript.previewTagEbene == -2 && getValOfSubProp(aEventSet, 'aId') && getValOfSubProp(aAntworten[aEventSet.aId], 'tags.length') > 0
                ? (transcript.showEventSetIds ? aEventSet.pk + ': ' : '') + getUsedTagEbenen(aAntworten[aEventSet.aId].tags)
                : (transcript.showEventSetIds ? aEventSet.pk : ''))
            }}
          &nbsp;</text>
        </g>
      </g>
